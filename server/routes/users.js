...
router.get('/:id/activities', async (req, res) => {
  try{
    const { rows } = await db.query(`select a.* from activity_members m join activities a on m.activity_id = a.id where m.user_id = $1 order by a.created_at desc`, [req.params.id])
    res.json(rows)
  }catch(err){
...
router.get('/:id/splits', async (req, res) => {
  try{
    const sql = `select s.*, e.title, e.activity_id from expense_splits s left join expenses e on s.expense_id = e.id where s.user_id = $1 order by s.created_at desc`
...
router.get('/:id/dashboard', async (req, res) => {
  try{
    ...
    const owedSql = `select coalesce(sum(amount),0) as total from expense_splits where owed_to = $1 and status = 'pending'`
    const oweSql = `select coalesce(sum(amount),0) as total from expense_splits where user_id = $1 and status = 'pending'`
    const { rows: owedRows } = await db.query(owedSql, [uid])
    const { rows: oweRows } = await db.query(oweSql, [uid])
    const paymentsSql = `select paid_by, paid_to, amount from payments where paid_by = $1 or paid_to = $1`
