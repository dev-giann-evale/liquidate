...
router.post('/register', async (req, res) => {
  ...
  const insertUserSql = 'insert into liquidate_users (email, password_hash) values ($1,$2) returning id, email'
...
router.post('/login', async (req, res) => {
  ...
  if(!email || !password) return res.status(400).json({ error: 'missing email or password' })
  try{
  const { rows } = await db.query('select id, email, password_hash from liquidate_users where email = $1', [email])
    const user = rows[0]
